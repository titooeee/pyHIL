...
from utilityScipts import channelDef
from utilityScipts import channelDictTemplate

class can_dict:
    
    def __init__(self,yml_path,can_shm_dict):
        self.yml_path = yml_path
        self.can_shm_dict = can_shm_dict
        self.mapDict = dict()
    ...
    def create_can_shm_template(self):
        ...
        for can_bus_config in can_config:
            ...
            for frame in can_bus_config['TX_LIST']:
                ...
                # channel/msg/signal format
                for sig in msg.signals:
                    # channelData = channelDef(f"{can_bus_config['Channel']}/{msg.name}/{sig.name}",0)
                    self.can_shm_dict[f"{can_bus_config['Channel']}/{msg.name}/{sig.name}"]= self.manager.dict(channelDictTemplate())
    ...
    def updateDict(self,startIndex):
            ...
            for can_bus_config in can_config:
                ...
                for frame in can_bus_config['TX_LIST']:
                    ...
                    for sig in msg.signals:
                        # channelData = channelDef(f"{can_bus_config['Channel']}/{msg.name}/{sig.name}",0)
                        self.mapDict[f"CAN/Tx/{can_bus_config['Channel']}/{msg.name}/{sig.name}"]= dict(channelDictTemplate())
                        self.mapDict[f"CAN/Tx/{can_bus_config['Channel']}/{msg.name}/{sig.name}"]['index']= localIndex
                        localIndex+=1
                ...
                for frame in can_bus_config['RX_LIST']:
                    ...
                    for sig in msg.signals:
                        # self.can_shm_dict[can_bus_config['Channel']][msg.name][sig.name]=0
                        # channelData = channelDef(f"{can_bus_config['Channel']}/{msg.name}/{sig.name}",0)
                        self.mapDict[f"CAN/Rx/{can_bus_config['Channel']}/{msg.name}/{sig.name}"]= dict(channelDictTemplate())
                        self.mapDict[f"CAN/Rx/{can_bus_config['Channel']}/{msg.name}/{sig.name}"]['index']= localIndex
                        localIndex+=1
            return self.mapDict
