...
class CANLogger:
    ...
    def setup_can_interface(self):
        try:
            self.bus = can.interface.Bus(channel=self.interface, bustype='nixnet', fd=True, receive_own_messages=True)
    ...
    def start_logging(self):
        ...
        self.logger = can.SizedRotatingLogger(self.log_file, max_bytes=2000*1024*1024)
...
class StateMachineServer:
    ...
    def handle_running(self, message):
        if message == "stop":
            self.stop_event.set()
            for p in self.can_processes:
                p.join()
            self.state = "IDLE"
            self.update_gui_callback(self.state)
            self.stop_event.clear()
            self.can_processes.clear()
...
class StateMachineGUI:
    def __init__(self, master):
        ...
        self.server = StateMachineServer(self.update_state, self.message_queue)
        self.zmq_thread = threading.Thread(target=self.zmq_polling_loop)
    ...
    def zmq_polling_loop(self):
        while True:
            try:
                message = self.zmq_socket.recv_string()
                if message == "quit-app":
                    self.zmq_socket.send_string("Quitting")
                    break
                response = self.server.handle_request(message)
                self.zmq_socket.send_string(response)
            except zmq.ZMQError:
